...
class UITable:
    ...
    def __init__(
        ...
        actions_buttons_factory: Callable[[tk.Misc], list[tk.Button]] = None,
        on_selected_rows_change=None,
        ...
        # Retrieve columns ids from rows if not specified
        if columns_ids is None:
            columns_ids = []
            if len(rows) > 0:
        ...
        if actions_buttons_factory is not None:
            self.__actions_buttons = actions_buttons_factory(top_frame)
            for i, action_button in enumerate(reversed(self.__actions_buttons)):
                action_button.pack(
                    side=tk.RIGHT,
                    padx=Constants.UI_PAD_BIG if i == 0 else Constants.UI_PAD_SMALL
                )
        ...
        for column_id in columns_ids:
            ...
            if column_id == Constants.UI_TABLE_KEY_COL_SELECTION:
                ...
                Constants.UI_TABLE_KEY_COL_ROMS_FOLDER,
                Constants.UI_TABLE_KEY_COL_PLATFORM,
                Constants.UI_TABLE_KEY_COL_ROM,
                Constants.UI_TABLE_KEY_COL_RESOURCE,
                Constants.UI_TABLE_KEY_COL_MEDIA
            ]:
                width = 100
                anchor = tk.CENTER
                stretch = False
        ...
        self.__when_selection_changed()

        self.__loaded = True

    def __when_selection_changed(self):
        """When selection rows changed"""
        ...
        if self.__loaded and self.__on_selected_rows_changed is not None:
