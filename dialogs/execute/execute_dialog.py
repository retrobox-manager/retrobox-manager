...
class ExecuteDialog:
    ...
    def __init__(
        ...
        # Add a sub progress bar if the feature sub items is enabled
        if self.__executor.is_sub_items_enabled():
            ...
            sub_progress_bar.pack(
                side=tk.TOP
            )

            self.sub_progress_label = tk.Label(
                self.dialog
            )
            self.sub_progress_label.pack(
                side=tk.TOP
            )
        else:
        ...
        self.__executor.set_ui_components(
            ...
            sub_progress_label=self.sub_progress_label,
